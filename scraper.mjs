...
// AYRIM NOKTASI 1: Tüm kayıt dosyaları "_1" uzantılı (JSON kuyruk takibi için yerelde kalmaya devam ediyor)
const COOKIE_FILE = path.join(__dirname, "cookies_1.json");
const PENDING_FILE = path.join(__dirname, "pending_1.json");
const COMPLETED_FILE = path.join(__dirname, "completed_1.json");
...
async function loadCookies(page) {
    if (fs.existsSync(COOKIE_FILE)) {
        try {
            ...
            console.log('🍪 Cookie\'ler yüklendi (Scraper 1).');
            return true;
...
async function topluMangaAktar(page, mangaSlug, mangaBaslik) {
    ...
    const bolumlerListesi = hamLinkler
    .map(url => ({ url, gercekNumara: extractChapterNumber(url) }))
    .filter(b => b.gercekNumara !== null)
    .sort((a, b) => Number(a.gercekNumara) - Number(b.gercekNumara));
    const seen = new Set();
    ...
    for (let idx = 0; idx < islenecekListe.length; idx++) {
        ...
        try {
            ...
            try {
                ...
                // Artık lokal klasör temizliğine gerek yok, veritabanı rollback yeterli
...
const mangaListesi = [
    { slug: 'the-beginning-after-the-end', baslik: 'The Beginning After The End' },
    ...
    { slug: 'solo-max-level-newbie', baslik: 'Solo Max Level Newbie' },
    { slug: 'manager-kim', baslik: 'Manager Kim' },
    { slug: 'a-not-so-fairy-tale', baslik: 'A Not So Fairy Tale' },
    { slug: 'hero-killer', baslik: 'Hero Killer' },
    { slug: 'i-want-to-be-fooled', baslik: 'I Want to Be Fooled' },
    { slug: 'myst-might-mayhem', baslik: 'Myst Might Mayhem' },
    { slug: 'the-villain-wants-to-live', baslik: 'The Villain Wants to Live' },
    { slug: 'disastrous-necromancer', baslik: 'Disastrous Necromancer' },
    { slug: 'every-returned-mercenary-has-a-plan', baslik: 'Every Returned Mercenary Has a Plan' },
    { slug: 'in-the-garden-of-may', baslik: 'In the Garden of May' },
    { slug: 'absolute-regression', baslik: 'Absolute Regression' },
    { slug: 'subzero', baslik: 'Subzero' },
    { slug: 'tale-of-a-scribe-who-retires-to-the-countryside', baslik: 'Tale of a Scribe Who Retires to the Countryside' },
    { slug: 'i-only-see-you', baslik: 'i Only See You' },
    { slug: 'reincarnator', baslik: 'Reincarnator' },
    { slug: 'the-divine-demon-s-grand-ascension', baslik: 'The Divine Demon\'s Grand Ascension' },
    { slug: 'regressing-with-the-king-s-power', baslik: 'Regressing with the King\'s Power' },
    { slug: 'death-god', baslik: 'Death God' },
    { slug: 'the-heavenly-demon-can-t-live-a-normal-life', baslik: 'The Heavenly Demon Can\'t Live a Normal Life' },
    { slug: 'the-ultimate-shut-in', baslik: 'The Ultimate Shut-In' },
    { slug: 'i-shall-master-this-family', baslik: 'I Shall Master This Family' },
    { slug: 'all-class-awakening-god-slayer', baslik: 'All Class Awakening: God Slayer' },
    { slug: 'helmut-the-forsaken-child', baslik: 'Helmut: The Forsaken Child' },
    { slug: 'the-tutorial-is-too-hard', baslik: 'The Tutorial Is Too Hard' },
    { slug: 'reformation-of-the-deadbeat-noble ', baslik: 'Reformation of the Deadbeat Noble ' },
    { slug: 'lookism', baslik: 'Lookism' },
    { slug: 'genius-archer-s-streaming', baslik: 'Genius Archer\'s Streaming' },
    { slug: 'prismatic-genius-magician', baslik: 'Prismatic Genius Magician' },

];

async function baslat() {
    console.log('\n🚀 Scraper 1 Başlatılıyor...\n');
    const userDataPath = path.join(__dirname, 'chrome_profil_1');
